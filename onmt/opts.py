...
def model_opts(parser):
    ...
    """
    # classifier
    group = parser.add_argument_group('Model-Classifier')
    group.add('--n_layers_cls', '-n_layers_cls', type=int, default=2, help='')
    group.add('--n_heads_cls', '-n_heads_cls', type=int, default=4, help='')
    group.add('--dim_each_head_cls', '-dim_each_head_cls', type=int, default=32, help='')
    group.add('--classifier_out_layer', '-classifier_out_layer', type=str, default='SentMeanPoolOutLayer', help='')
    group.add('--num_styles', '-num_styles', type=int, default=2, help='')
    group.add(
        '--faketruthstyle_mode', '-faketruthstyle_mode', type=str2bool, default=True,
        help='discriminator whether use the mode of: 2 * num_styles'
    )
    group.add(
        '--force_positive_gain', '-force_positive_gain', type=str2bool, default=True,
        help='force gain/loss to be positive (non-negative)'
    )
    group.add(
        '--force_positive_gain_mode_t', '-force_positive_gain_mode_t', type=str, default='normal',
        help='force gain/loss to be positive (non-negative)', choices=['normal', 'exp', 'sigmoid', 'sigmoid0', 'relu', 'relu0']
    )
    group.add(
        '--force_positive_gain_mode_s', '-force_positive_gain_mode_s', type=str, default='normal',
        help='force gain/loss to be positive (non-negative)', choices=['normal', 'exp', 'sigmoid', 'sigmoid0', 'relu', 'relu0']
    )
    group.add(
        '--force_positive_gain_ratio', '-force_positive_gain_ratio', type=float, default=1.0,
        help='force gain/loss to be positive: ratio'
    )
    group.add(
        '--label_tokens', '-label_tokens', type=str, default='<sl0>,<sl1>',
        help='the label tokens'
    )

    group = parser.add_argument_group('Module-StyleGenerator')
    group.add('--n_layers_sgenerator', '-n_layers_sgenerator', type=int, default=2, help='')
    group.add('--n_heads_sgenerator', '-n_heads_sgenerator', type=int, default=4, help='')
    group.add('--dim_each_head_sgenerator', '-dim_each_head_sgenerator', type=int, default=32, help='')

    group = parser.add_argument_group('Module-TextDiscriminator')
    group.add('--n_layers_tdiscr', '-n_layers_tdiscr', type=int, default=2, help='')
    group.add('--n_heads_tdiscr', '-n_heads_tdiscr', type=int, default=4, help='')
    group.add('--dim_each_head_tdiscr', '-dim_each_head_tdiscr', type=int, default=32, help='')
    group = parser.add_argument_group('Module-StyleDiscriminator')
    group.add('--n_layers_sdiscr', '-n_layers_sdiscr', type=int, default=2, help='')
    group.add('--n_heads_sdiscr', '-n_heads_sdiscr', type=int, default=4, help='')
    group.add('--dim_each_head_sdiscr', '-dim_each_head_sdiscr', type=int, default=32, help='')
...
def train_opts(parser):
    ...
    # train dec
    group = parser.add_argument_group('train_wh')
    group.add('--print_args', '-print_args', type=str2bool, default=False)
    group.add('--max_dec_steps', '-max_dec_steps', type=int, default=40)
    group.add('--init_steps', '-init_steps', type=int, default=1000)
    group.add('--learn_disc_steps', '-learn_disc_steps', type=int, default=10)
    group.add('--learn_disc_increase', '-learn_disc_increase', type=float, default=0)
    group.add('--apply_disc_increase', '-apply_disc_increase', type=float, default=0)
    group.add('--learn_reconst_increase', '-learn_reconst_increase', type=float, default=0)
    group.add('--main_steps', '-main_steps', type=int, default=5)
    group.add('--reconstruct_steps', '-reconstruct_steps', type=int, default=1)
    group.add('--reconstruct_every_step', '-reconstruct_every_step', type=str2bool, default=False)
    group.add('--ap_gen_self_style_text', '-ap_gen_self_style_text', type=str2bool, default=False)
    group.add('--cls_restore_path', '-cls_restore_path', type=str, default=None)
    group.add('--show_model_struct', '-show_model_struct', action='store_true')
    group.add('--sr_noise_prob', '-sr_noise_prob', type=float, default=0.3, help='the noise prob for self reconstruction')
    group.add('--use_recycle', '-use_recycle', action='store_true')
    group.add('--force_cycle_len', '-force_cycle_len', type=str2bool, default=False)
    group.add('--no_recycle_to_fw', '-no_recycle_to_fw', action='store_true')
    group.add('--occupy_ram', '-occupy_ram', action='store_true')
    group.add('--optimizer_class', '-optimizer_class', type=str, default='Adam')
    group.add('--reconst_optimizer_class', '-reconst_optimizer_class', type=str, default='Adam')
    group.add('--use_cls_feedback', '-use_cls_feedback', action='store_true')
    group.add('--sent_stop_aware', '-sent_stop_aware', action='store_true')
    group.add('--discr_sent_stop_aware', '-discr_sent_stop_aware', type=str2bool, default=False, help='Discriminator get aware of the stop of text')
    group.add('--force_ap_text_stop', '-force_ap_text_stop', type=str2bool, default=False, help='vocab loss')
    group.add('--vloss_token_mode', '-vloss_token_mode', type=str2bool, default=True, help='token mode or batch mode')
    group.add('--cycle_loss_ratio', '-cycle_loss_ratio', type=float, default=1.0)
    group.add('--sr_loss_ratio', '-sr_loss_ratio', type=float, default=1.0, help='self reconstruction loss ratio')
    group.add('--cls_loss_ratio', '-cls_loss_ratio', type=float, default=1.0, help='')
    group.add('--without_disc_init', '-without_disc_init', action='store_true')
    group.add('--restore_from', '-restore_from', type=str, default=None)
    group.add('--restore_step', '-restore_step', type=int, default=None)
    group.add('--dynamic_lr', '-dynamic_lr', action='store_true')
    group.add('--without_bp2stlgen', '-without_bp2stlgen', action='store_true', help='no bp to style generator while training main model')
    group.add('--without_stl_penalty', '-without_stl_penalty', action='store_true', help='Does not penalty the element gap between generated style infos and real infos')
    group.add('--style_train_steps', '-style_train_steps', type=int, default=-1)
    group.add('--dynamic_dec_type', '-dynamic_dec_type', type=str, default='use_ori_train_dec',
              choices=['use_ori_train_dec', 'use_discrete_accum_dec', 'use_official_dec', 'use_accum_dec'])

    group.add('--recycle_steps', '-recycle_steps', type=int, default=1)
    group.add('--n_show_gen_texts', '-n_show_gen_texts', type=int, default=20)
    group.add('--style_ap_scale', '-style_ap_scale', type=float, default=1.0)
    group.add('--text_ap_scale', '-text_ap_scale', type=float, default=1.0)
    group.add('--start_peak_step', '-start_peak_step', type=int, default=-1,
              help='during the peak training stage, reset the main training step')
    group.add('--peak_main_steps', '-peak_main_steps', type=int, default=5)
    group.add('--use_gp_norm_seq', '-use_gp_norm_seq', type=str2bool, default=False)
    group.add('--convert_gain2loss', '-convert_gain2loss', type=str2bool, default=False)
    # gradient alpha
    group.add('--style_gen_grad_alpha', '-style_gen_grad_alpha', type=float, default=0.01)
    group.add('--text_gen_grad_alpha', '-text_gen_grad_alpha', type=float, default=0.01)
    group.add('--style_disc_grad_alpha', '-style_disc_grad_alpha', type=float, default=1.0)
    group.add('--text_disc_grad_alpha', '-text_disc_grad_alpha', type=float, default=1.0)
    # --
    group.add('--alpha_reconst', '-alpha_reconst', type=float, default=1.0)
    group.add('--alpha_text_ap', '-alpha_text_ap', type=float, default=1.0)
    group.add('--alpha_text_cls', '-alpha_text_cls', type=float, default=1.0)
    group.add('--alpha_recycle_fw', '-alpha_recycle_fw', type=float, default=1.0)
    group.add('--alpha_recycle', '-alpha_recycle', type=float, default=1.0)
    group.add('--alpha_style_ap', '-alpha_style_ap', type=float, default=1.0)
    group.add('--alpha_style_cls', '-alpha_style_cls', type=float, default=1.0)
    group.add('--alpha_style_gap', '-alpha_style_gap', type=float, default=1.0)
    # fuse protection
    group.add('--fuse_protect', '-fuse_protect', type=str2bool, default=False)
    group.add('--fuse_text_min', '-fuse_text_min', type=float, default=0.0)
    group.add('--fuse_text_max', '-fuse_text_max', type=float, default=0.0)
    group.add('--fuse_style_min', '-fuse_style_min', type=float, default=0.0)
    group.add('--fuse_style_max', '-fuse_style_max', type=float, default=0.0)
    # wgan setting
    group.add('--wgan_text_k', '-wgan_text_k', type=float, default=1.0)
    group.add('--wgan_text_lamb', '-wgan_text_lamb', type=float, default=10.0)
    group.add('--wgan_style_k', '-wgan_style_k', type=float, default=1.0)
    group.add('--wgan_style_lamb', '-wgan_style_lamb', type=float, default=10.0)
    # dynamic decoding temperature
    group.add('--d_g_use_temperature', '-d_g_use_temperature', type=str2bool, default=False)
    group.add('--dec_temperature_startval', '-dec_temperature_startval', type=float, default=1.0, help='init temperature')
    group.add('--dec_temperature_changeepoch', '-dec_temperature_changeepoch', type=float, default=1.0, help='interval epoch')
    group.add('--dec_temperature_changeval', '-dec_temperature_changeval', type=float, default=-1.0, help='interval step')
    # the loading dataset order while restore and continue training
    group.add('--continue_train_data_id', '-continue_train_data_id', type=int, default=0)

    # config for translator during training
    group = parser.add_argument_group('train_wh_translator')
    group.add('--trans_alpha', '-trans_alpha', type=float, default=0.,
              help="Google NMT length penalty parameter "
                   "(higher = longer generation)")
    group.add('--trans_beta', '-trans_beta', type=float, default=-0.,
              help="Coverage penalty parameter")
    group.add('--trans_length_penalty', '-trans_length_penalty', default='none',
              choices=['none', 'wu', 'avg'],
              help="Length Penalty to use.")
    group.add('--trans_coverage_penalty', '-trans_coverage_penalty', default='none',
              choices=['none', 'wu', 'summary'],
              help="Coverage Penalty to use.")
    group.add('--trans_n_best', '-trans_n_best', type=int, default=1,
              help="If verbose is set, will output the n_best "
                   "decoded sentences")
    group.add('--trans_min_length', '-trans_min_length', type=int, default=0,
              help='Minimum prediction length')
    group.add('--trans_max_length', '-trans_max_length', type=int, default=100,
              help='Maximum prediction length.')
    group.add('--trans_ratio', '-trans_ratio', type=float, default=-0.,
              help="Ratio based beam stop condition")
    group.add('--trans_beam_size', '-trans_beam_size', type=int, default=5,
              help='Beam size')
    group.add('--trans_random_sampling_topk', '-trans_random_sampling_topk',
              default=1, type=int,
              help="Set this to -1 to do random sampling from full "
                   ...
                   "search.")
    group.add('--trans_random_sampling_temp', '-trans_random_sampling_temp',
              default=1., type=float,
              help="If doing random sampling, divide the logits by "
                   "this before computing softmax during decoding.")
    group.add('--trans_stepwise_penalty', '-trans_stepwise_penalty', action='store_true',
              help="Apply penalty at every decoding step. "
                   "Helpful for summary penalty.")
    group.add('--trans_block_ngram_repeat', '-trans_block_ngram_repeat',
              type=int, default=0,
              help='Block repetition of ngrams during decoding.')
    group.add('--trans_ignore_when_blocking', '-trans_ignore_when_blocking',
              nargs='+', type=str, default=[],
              help="Ignore these strings when blocking repeats. "
                   "You want to block sentence delimiters.")
    group.add('--trans_replace_unk', '-trans_replace_unk', action="store_true",
    ...
    group.add('--trans_phrase_table', '-trans_phrase_table', type=str, default="",
...
def translate_opts(parser):
    ...
    group = parser.add_argument_group('StyleSetting')
    group.add('--desired_style', '-desired_style', type=int, default=0)
    group.add('--vocab_opt_path', '-vocab_opt_path', type=str, required=True)
