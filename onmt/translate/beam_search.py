...
class BeamSearch(DecodeStrategy):
    ...
    def initialize(self, memory_bank, src_lengths, src_map=None, device=None,
        ...
        self.alive_example_ids = tile(self.alive_example_ids, self.beam_size, dim=0)
    ...
    def advance(self, log_probs, attn, dec_out):
        """
        :param log_probs:
        :param attn:
        :param dec_out: [1, batch_size, dim]
        :return:
        """
        vocab_size = log_probs.size(-1)
        ...
        # Append last predicted output state
        current_dec_out = dec_out.index_select(1, self.select_indices)
        if 1 == step:
            self.alive_output_state_seq = current_dec_out
        else:
            self.alive_output_state_seq = self.alive_output_state_seq.index_select(1, self.select_indices)
            self.alive_output_state_seq = torch.cat([self.alive_output_state_seq, current_dec_out], 0)
    ...
    def update_finished(self):
        ...
        pred_dec_outs = self.alive_output_state_seq.view(
            step-1, _B_old, self.beam_size, self.alive_output_state_seq.size(-1)
        )
        attention = (
        ...
        for i in range(self.is_finished.size(0)):  # Batch level
            ...
            if finish_flag and len(self.hypotheses[b]) >= self.n_best:
                ...
                for n, (score, pred, pred_out_state, attn) in enumerate(best_hyp):
                    if n >= self.n_best:
                        break
                    self.scores[b].append(score)
                    self.predictions[b].append(pred)  # ``(batch, n_best,)``
                    self.output_states[b].append(pred_out_state)
        ...
        self.alive_output_state_seq = pred_dec_outs.index_select(1, non_finished).view(
            step - 1, _B_new * self.beam_size, self.alive_output_state_seq.size(-1))
        self.alive_example_ids = self.alive_example_ids.index_select(0, non_finished)
