...
class GreedySearch(DecodeStrategy):
    ...
    def advance(self, log_probs, attn, dec_out):
        ...
        if 1 == step:
            self.alive_output_state_seq = dec_out
        else:
            self.alive_output_state_seq = torch.cat([self.alive_output_state_seq, dec_out], 0)
    ...
    def update_finished(self):
        ...
        for b in finished_batches.view(-1):
            ...
            self.output_states[b_orig].append(self.alive_output_state_seq[:, b])
        ...
        self.alive_output_state_seq = self.alive_output_state_seq[:, is_alive]
        self.alive_example_ids = self.alive_example_ids[is_alive]
