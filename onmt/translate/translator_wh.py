...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    vocab_model_opt_ckpt = torch.load(opt.vocab_opt_path)
    fields, model_opt = vocab_model_opt_ckpt['fields'], vocab_model_opt_ckpt['model_opt']
    if opt.gpu:
        model_ckpt = torch.load(opt.model_path, map_location=lambda storage, loc: storage)
    else:
        model_ckpt = torch.load(opt.model_path)
    model, style_generator = model_ckpt['main_model'].eval(), model_ckpt['style_generator'].eval()
    if opt.gpu < 0:
        model = model.cpu()
        style_generator = style_generator.cpu()
...
def post_handle_sent(sents):
    results = []
    dot_char = '.'
    excalmatory_char = '!'

    for sent in sents:
        sent = delete_repeat(sent)
        cur_sent_len = len(sent)
        dot_idx = cur_sent_len
        excalmatory_idx = cur_sent_len
        if dot_char in sent:
            dot_idx = sent.index(dot_char)
        if excalmatory_char in sent:
            excalmatory_idx = sent.index(excalmatory_char)
        cur_len = min((cur_sent_len, dot_idx if dot_idx > 1 else cur_sent_len, excalmatory_idx if excalmatory_idx > 1 else cur_sent_len))
        temp = sent[:int(cur_len)+1].split()
        if len(temp) <= 1:
...
class Translator(object):
    def __init__(self,
        ...
        self.desired_style = desired_style
        self.fields = fields
        ...
        self.style_generator.to(self._dev)
    ...
    def from_opt(
        ...
        """
        desired_style = opt.desired_style
    ...
    def translate(
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                n_best_preds = post_handle_sent(n_best_preds)
                ...
                if self.verbose:
                    new_pred_sents = post_handle_sent(trans.pred_sents)
                    trans.pred_sents = new_pred_sents
    ...
    def _align_forward(self, batch, predictions, desired_style=None):
        """
        ...
        # (1) Encoder forward.
        src, enc_states, memory_bank, src_lengths = self._run_encoder(batch, desired_style=desired_style)
    ...
    def translate_batch(self, batch, src_vocabs, attn_debug, desired_style=None):
        ...
        with torch.no_grad():
            ...
            return self._translate_batch_with_strategy(batch, src_vocabs,
                                                       decode_strategy, desired_style=desired_style)

    def _run_encoder(self, batch, desired_style=None):
        """
        :param batch:
        :param desired_style: [batch_size]
        :return:
        """
        src, src_lengths = batch.src if isinstance(batch.src, tuple) \
                           else (batch.src, None)
        cur_batch_size = src.size(1)
        # pat_src = torch.LongTensor([[self.desired_style] * cur_batch_size]).unsqueeze(2).type_as(src)
        # pat_src = torch.cat((pat_src, src), 0)
        # pat_src_lengths = src_lengths + 1
        if desired_style is None:
            desired_style_tensor = torch.LongTensor([[self.desired_style] * cur_batch_size]).squeeze(0).to(self._dev)
        else:
            desired_style_tensor = desired_style
        desired_style_represents = self.style_generator(desired_style_tensor, src, src_lengths)
        enc_states, memory_bank, src_lengths = self.model.encoder(
            src, src_lengths)
        new_memory_bank = self.model.bond_layer(memory_bank, desired_style_represents)
    ...
    def _translate_batch_with_strategy(
            ...
            batch,
            src_vocabs,
            decode_strategy, desired_style=None):
    ...
    def _translate_batch_with_strategy_for_train(
            self,
            batch,
            src_vocabs,
            decode_strategy,
        ...
        Returns:
            results (dict): The translation results.
        """
        assert self.copy_attn is not True, "No Support for copying anymore"
        # (0) Prep the components of the search.
        use_src_map = self.copy_attn
        parallel_paths = decode_strategy.parallel_paths  # beam_size
        batch_size = src_lengths.size(0)
        cur_max_dec_steps = max_dec_steps if max_dec_steps is not None else decode_strategy.max_length
        ...
        # (3) Begin decoding step by step:
        for step in range(cur_max_dec_steps):
            decoder_input = decode_strategy.current_predictions.view(1, -1, 1)
            ...
            # INFO 训练过程强制需要有结束
            any_finished = decode_strategy.is_finished.any()
            if step == cur_max_dec_steps - 1:
                decode_strategy.is_finished = decode_strategy.is_finished.fill_(True)
                any_finished = True
            if any_finished:
                decode_strategy.update_finished()
                if decode_strategy.done:
        ...
        results["scores"] = decode_strategy.scores
        results["predictions"] = decode_strategy.predictions
        results["output_states"] = decode_strategy.output_states
        # print("=" * 80)
        # # print('results["predictions"]: len1 :: {}, len2 :: {}, size :: {}'.format(
        # #     len(results["predictions"]), len(results["predictions"][0]), results["predictions"][0][0].size()
        # # ))
        # print('results["predictions"]')
        # print('len1 :: {}'.format(len(results["predictions"])))
        # print('len2 :: {}'.format(len(results["predictions"][0])))
        # print('size :: {}'.format(results["predictions"][0]))
        # print('results["output_states"]: len1 :: {}, len2 :: {}, size :: {}'.format(
        #     len(results["output_states"]), len(results["output_states"][0]), results["output_states"][0][0].size()
        # ))
        results["attention"] = decode_strategy.attention
        # if self.report_align:
        #     results["alignment"] = self._align_forward(
    ...
    def translate_batch_for_train(self, src, enc_states, memory_bank, src_lengths, max_dec_steps=None):
        """Translate a batch of sentences.
            desired_style: [batch_size]
        """
        attn_debug = False
        # without using copy, it can be empty
        src_vocabs = []
        batch_size = src_lengths.size(0)
        batch = None
        ...
        with torch.no_grad():
            if self.beam_size == 1:
                ...
                decode_strategy = BeamSearch(
                    ...
                    stepwise_penalty=self.stepwise_penalty,
                    ratio=self.ratio)
            return self._translate_batch_with_strategy_for_train(
