...
def _build_field_vocab(field, counter, size_multiple=1, num_styles=2, **kwargs):
...
def _build_fv_from_multifield(multifield, counters, build_fv_args,
                              size_multiple=1, num_styles=2):
    for name, field in multifield:
        _build_field_vocab(
            field,
            counters[name],
            size_multiple=size_multiple,
            num_styles=num_styles,  # INFO: UPDATED BY AZ HERE
            **build_fv_args[name]
        )
...
class OrderedIterator(torchtext.data.Iterator):
    ...
    def data(self):
        ...
        # INFO we need to random shuffle the dataset at training time
        if self.train:
            xs = [self.dataset[i] for i in self.random_shuffler(range(len(self.dataset)))]
        elif self.sort:
            xs = sorted(self.dataset, key=self.sort_key)
...
class DatasetLazyIter(object):
    ...
    def __init__(self, dataset_paths, fields, batch_size, batch_size_fn,
        ...
        self.pool_factor = pool_factor
        logger.info("-" * 80)
        logger.info('Dataset files: %s' % self._paths)
    ...
    def _iter_dataset(self, path):
        ...
        cur_iter = OrderedIterator(
            ...
            device=self.device,
            train=self.is_train,
            # INFO must be False, because we need to shuffle dataset at training time
            sort=False,
...
def build_dataset_iter(corpus_type, fields, opt, is_train=True, multi=False, start_shard_id=0):
    """
    ...
    if 0 != start_shard_id:
        start_shard_id_ = start_shard_id % len(dataset_paths)
        logger.info("shifting and starting with start_shard_id: {}".format(start_shard_id_))
        dataset_paths = dataset_paths[start_shard_id_:] + dataset_paths[:start_shard_id_]
